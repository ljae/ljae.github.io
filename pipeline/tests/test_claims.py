...
@pytest.mark.parametrize("text", [
    # 손대다 → 손대기. 신고 cd-c31f042d — 원문에서 한 낱말이라 _contiguous 는
    # 통과하고 SEL_NOT_AFTER 는 뒤만 봤다.
    "가나수학학원 레테는 초등 저학년이 손대기 어려운 고난도 시험이라기보다",
    # 앞 글자와 '대' 가 낱말을 이룬다: 강대·의대·경북대·일대·최대 (9/7 근거 실측)
    "가나수학학원 다니다가 강대기숙 들어갔어요",
    "가나수학학원 옆 의대기숙에서 5주간 수업",
    "가나수학학원 해도 경북대기준으로 충분해요",
    "가나수학학원 원장님의 주거 일대기 현재 거주 중인",
    "가나수학학원 최대기대작",
    # X대다 → X대기 (덧대다·투덜대다)
    "가나수학학원 자재를 덧대기도 하고",
    "가나수학학원 숙제가 많다고 투덜대기는 하지만",
    # 뒤 글자가 '기' 와 합쳐 다른 낱말: 대기업 (9/7 근거 4건)
    "가나수학학원 강사님은 대기업에서 20년을 일하시고",
])
def test_daegi_inside_another_word_is_not_a_waitlist(text):
    """★ 사건 낱말이 남의 낱말 안쪽에서 생기던 결함 — 앞을 보는 장치가 없었다."""
    assert "sel.waitlist" not in events(text)


@pytest.mark.parametrize("text", [
    # 붙여 쓴 합성어 — 앞 낱말이 '무엇을 기다리는지' 를 말한다 (코퍼스 실측)
    "가나수학학원 수업대기 평균 2년이에요",
    "가나수학학원 #레벨테스트대기중",
    "가나수학학원 등록대기 걸었어요",
    "가나수학학원 레테대기 두 달이래요",
    "가나수학학원 학원대기해야 한다고 하네요",
    "가나수학학원 수업 예약대기를 걸어놓을 수 있어요",
    "가나수학학원 마감대기등록 버튼이 보여요",
    "가나수학학원 중등기본 월금(2인대기)",
    # 띄어 쓴 보통 꼴
    "가나수학학원 대기 걸었어요",
    "가나수학학원 대기번호 받았어요",
])
def test_compound_waitlists_survive(text):
    """반대쪽. 앞 글자를 무조건 막으면 이것들이 죽는다 — 여기가 깨지면 과하게 조인 것."""
    assert "sel.waitlist" in events(text)
