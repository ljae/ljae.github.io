...
def test_관은_합치고_과목은_가른다_같은_주소여도():
    """목동 정상은 셋이 **같은 주소**(목동서로 349)에 있다. 그중 둘은
    합쳐야 할 관이고 하나는 갈라야 할 다른 과목이다 — 주소만 보면 셋 다
    한 학원처럼 보인다. 통합이 양쪽으로 틀릴 수 있다는 것을 이 한 짝이
    보여 준다.

        목동1관정상어학원 + 목동3관정상어학원  → 합친다 (같은 브랜드·과목의 관)
        목동정상수학학원                        → 가른다 (과목이 다르다)
    """
    addr = "서울 양천구 목동서로 349"
    rows = [rec("목동1관정상어학원", addr, aid="24080"),
            rec("목동3관정상어학원", addr, aid="13544"),
            rec("목동정상수학학원", addr, aid="23351")]
    assert grouped(rows, "목동1관정상어학원", "목동3관정상어학원")
    assert not grouped(rows, "목동1관정상어학원", "목동정상수학학원")
