...
SEL_EVENTS: tuple[tuple[str, float, tuple[str, ...], bool], ...] = (
    ...
    #
    # ★ 무게를 0.30 → 0.10 으로 내렸다 (2026-09-05 결정 D1-B).
    #   전수 조사(2026-09-04): 사건 3,242건의 **89%(2,885건)** 가 이 종류였고,
    #   점수가 있는 152곳 중 66곳(43%)이 **이것 하나만** 가지고 있었다.
    #   대치·목동 학원가에서 레테는 거의 보편적이라 난이도를 가르지 못한다.
    #   그런데도 이 기둥은 실효 기여 6.10 으로 평판(1.82)의 3.4배였고,
    #   그 값의 73%가 표본 크기로 설명됐다(log(표본)과 r=0.73) — 많이
    #   회자되는 학원일수록 누군가는 '레테 봤다' 고 쓰기 때문이다.
    #   빼지 않고 남기되(약한 신호도 신호다) 무게로 자리를 낮춘다.
    ("sel.test_exists", 0.10,
