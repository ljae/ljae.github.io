...
# 예체능으로 분류할 NEIS 분야구분.
# 기예(대) 는 실용음악·무용·미술·댄스·연기다 — 아이들이 다니는 곳이라 남긴다.
ARTS_REALMS = {"예능(대)", "기예(대)"}

# 기타. 바둑·로봇·코딩·속독·만화처럼 아이 대상이다.
# '정보' 는 두 곳뿐인데 둘 다 어린이 코딩학원이라 여기 둔다.
OTHER_REALMS = {"기타(대)", "정보"}
...
# ★ 성인 대상 직업·전문 학원은 등록부에서 아예 뺀다.
#
# 이 서비스는 초·중·고 학부모가 본다. 미용·승무원·간호조무사·바리스타·
# 부동산경매·애견미용(직업기술 128곳)과 편입·변리사·공인중개사·회계·
# MBA·성인고시(인문사회(대) 57곳)는 그 학부모가 찾는 학원이 아니다.
# 랭킹에 섞이면 '반포 기타 고등' 같은 목록이 성인 학원으로 채워진다.
#
# 분야구분으로 거른다 — 이름으로 거르면 '게임캔버스'·'디자인센트럴' 처럼
# 판단이 안 서는 것이 남고, 반대로 '미용' 이 든 아이 대상 학원을 잘못
# 지운다. 학원이 스스로 등록한 분야가 가장 분명한 신호다.
#
# 독서실(139곳)도 계속 뺀다 — 학원이 아니다.
...
# '성인취미' 는 이름 그대로 성인 대상이다(성인취미미술아트아뜰리에학원).
# '성인' 만으로 거르지 않는다 — '성인고시' 는 과정으로 이미 거르고, 낱말
# 하나짜리 필터는 오탐을 확인할 수 없다.
VOCATIONAL_NAME_WORDS = ("편입", "승무원", "미용", "메이크업", "네일", "바리스타",
                         "성인취미")


# 학습지·방문수업 프랜차이즈. **랭킹 대상이 아니다** (2026-09-05 결정).
#
# 학원이 아니라 공부방·방문수업이다. 등록명이 성격을 그대로 말한다 —
# '목동13단지…', '잠실엘스…', '한우리잠실리센츠…', '…래미안아이파크점…',
# '…독서교실…'. 아파트 단지 안의 교습소이고 근거 글도 그렇게 적혀 있다:
# '<<방문수업>> 집으로 찾아가는 한우리 독서토론 논술'.
#
# 실측(2026-09-04): 한우리 넷이 국어 2~5위였다(표본 130~137). 등록부에
# 같은 성격이 31곳이다. **NEIS 분야로는 안 갈린다** — 전부 '보습'·'논술'
# 교습소로 등록돼 있어 이름이 유일한 신호다.
#
# ★ 등록부에서 지우지 않는다. 학부모가 검색하면 나와야 하고, 화면에는
#   '이 랭킹의 대상이 아님' 이라고 적는다. **'낮은 점수' 가 아니라
#   '이 줄에 세우지 않는다' 는 뜻이다.**
HOMESCHOOL_FRANCHISES = (
    "한우리독서토론", "한우리독서교실", "한우리논술", "한우리서초", "한우리강남",
    "눈높이", "웅진씽크빅", "씽크빅", "기탄", "구몬", "재능스스로",
    "빨간펜", "아이스크림홈런", "밀크티", "와이즈캠프",
)


def is_homeschool_franchise(row: dict) -> bool:
    """학습지·방문수업 프랜차이즈인가. 이름으로만 판단한다."""
    name = (row.get("name") or "").replace(" ", "")
    return any(w in name for w in HOMESCHOOL_FRANCHISES)
