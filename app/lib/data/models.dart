...
class Score {
  ...
  /// 두 가지가 같은 결론에 이른다.
  ///  - 표본 0: 아직 아무 근거도 없다.
  ///  - 학술 과목인데 **진입난이도가 없다**: 네 기둥을 다 못 채웠다.
  ///
  /// 뒤쪽을 총점으로 내면 안 되는 이유가 실측으로 드러났다. 예전에는
  /// 남은 기둥으로 가중치를 다시 나눴는데, 그러면 **없는 기둥이 유리해
  /// 진다** — 모닝에듀(투명 100 · 표본 13)가 진입난이도가 없다는 이유로
  /// 투명성 몫을 15% → 23% 로 받아, 진입난이도 56.9 를 실제로 가진
  /// 렉스김어학원(표본 127)을 총점에서 앞질렀다.
  ///
  /// 예체능·기타는 두 기둥 산식이 정본이라 여기 걸리지 않는다.
  bool get hasTotal =>
      sampleSize > 0 && (subjectGroup != 'academic' || selectivity != null);
