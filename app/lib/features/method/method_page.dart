              ...
              child: Column(
                ...
                children: [
                  ...
                  _Prose(
                    ...
                    '쓰였나"가 아니라서, 최신성이나 최근 90일 언급 수, 언급량 추세 계산에는 '
                    '쓰지 않습니다.',
                  ),

                  const SizedBox(height: AppSpace.xl),
                  const SectionHeader('과목마다 따로 셉니다'),
                  const _Prose(
                    '한 학원이 여러 과목을 가르치는 것은 정상입니다. 하지만 랭킹은 '
                    '과목마다 따로 매깁니다. 수학 후기가 수백 건인 종합학원이 그 점수를 '
                    '그대로 들고 과학 랭킹 상위에 오르면, 그 순위는 "이 학원의 과학이 '
                    '좋다"가 아니라 "이 학원이 유명하다"를 뜻하게 되기 때문입니다.\n\n'
                    '그래서 후기를 과목별로 가릅니다. 그 과목을 말한 후기만 그 과목의 '
                    '근거로 씁니다. 과목을 밝히지 않은 후기는 그 학원의 대표 과목에만 '
                    '넣습니다 — 어느 과목인지 모르는 글을 모든 과목의 근거로 치는 것은 '
                    '근거 없는 확대입니다.\n\n'
                    '과목 판별은 학원 이름 근처(앞뒤 45자)에 나온 과목어만 봅니다. 글 '
                    '전체에서 찾으면 지역 잡담이 전부 그 학원의 근거가 됩니다 — 실제로 '
                    '"대치동 부동산 가격형성요인" 같은 글이 과학 근거로 잡혀 있었습니다.\n\n'
                    '상대 평가도 같은 과목끼리만 합니다. 표본 부족(유효 후기 10건 미만) '
                    '판정도 과목별로 따로 봅니다. 학원 상세에서 과목별 점수와 표본 수를 '
                    '함께 보실 수 있습니다.',
                  ),

                  const SizedBox(height: AppSpace.xl),
                  const SectionHeader('테크트리 단계에 학원을 어떻게 붙이나요'),
                  const _Prose(
                    '단계 구성은 저희가 직접 작성한 것이지만, 어느 학원이 어느 단계를 '
                    '담당하는지는 대부분 공시에 적혀 있지 않습니다. 그래서 근거를 세 '
                    '등급으로 나누고, 목록의 각 학원에 어느 등급인지 함께 적습니다.\n\n'
                    '아무 표시가 없으면 저희가 직접 확인해 연결한 것입니다. '
                    '"교습과정에 단서"는 학원 이름이나 NEIS 교습과정에 그 단계를 가리키는 '
                    '말이 있다는 뜻입니다(예: 이름에 "올림피아드"가 있으면 경시 단계). '
                    '"과목·학년으로 추정"은 그런 단서가 없어 과목과 대상 학년만 보고 그 '
                    '구간의 가장 일반적인 단계에 둔 것입니다.\n\n'
                    '예전에는 이 셋을 구분하지 않았고, 게다가 단서가 없는 학원을 세부 '
                    '단계에 임의로 흩뿌리고 있었습니다. 그 결과 "영재고·과학고 대비" 같은 '
                    '단계에 들어간 학원의 대부분이 근거 없이 그 자리에 있었습니다. '
                    '실명으로 영업하는 학원을 근거 없이 특정 과정 전문이라고 적을 수는 '
                    '없어서 그 배정을 없앴습니다.\n\n'
                    '대신 근거 있는 학원이 몇 곳뿐인 단계에서는 같은 과목·학년 구간의 '
                    '학원을 이어서 보여드리고, "이 단계 근거 없음 · 같은 구간"이라고 '
                    '적어 둡니다. 없는 근거를 지어내지 않으면서 "이 구간에 어떤 학원이 '
                    '있는가"에는 답하기 위해서입니다.',
                  ),

                  const SizedBox(height: AppSpace.xl),
                  const SectionHeader('지점을 가려서 셉니다'),
                  const _Prose(
                    '유명 학원은 여러 곳에 지점이 있습니다. 그런데 후기는 브랜드 이름으로 '
                    '쓰이기 때문에, 그대로 두면 분당 지점 후기가 대치 지점의 점수가 됩니다. '
                    '실제로 한 어학원의 대치 지점 언급 377건 중 60건이 다른 지역 글이었습니다.\n\n'
                    '그래서 두 가지 규칙을 둡니다. 글이 지역을 밝히면 그 권역의 지점만 '
                    '근거로 삼습니다. 저희가 다루는 네 학군 밖(분당·평촌·송도 등)을 '
                    '가리키는 글은 어느 지점에도 넣지 않습니다.\n\n'
                    '글이 지역을 밝히지 않으면 이름이 걸리는 지점 전부에 똑같이 반영합니다. '
                    '한 곳에만 몰아주면 그것은 추측이고, 통째로 버리면 브랜드에 대한 근거가 '
                    '사라집니다. 이렇게 반영된 글은 근거 목록에 "지점 불명 · 브랜드 공통"으로 '
                    '표시해 두었습니다 — 같은 글이 여러 지점에 보이는 것은 오류가 아닙니다.\n\n'
                    '지역 판별은 제목을 우선합니다. 본문에 나오는 지역 이름은 다른 학원의 '
                    '상호에 박혀 있는 경우가 많기 때문입니다.',
                  ),
                  ...
                  const SizedBox(height: AppSpace.xl),
                  const SectionHeader('학부모가 그리는 이 학원은 어떻게 만드나요'),
                  const _Prose(
                    '학원 상세의 "학부모가 그리는 이 학원"은 그 학원의 근거 발췌를 AI가 읽고 '
                    '네 칸 — 레벨테스트·숙제·커리큘럼·잘 맞는 아이 — 으로 적은 것입니다. '
                    '문장마다 원문 인용을 붙이게 하고, 그 인용이 원문에 글자 그대로 없으면 '
                    '문장째 버립니다.\n\n'
                    '한 칸은 서로 다른 글 2건 이상이 받쳐야 채웁니다. 모자라면 "아직 근거 부족"이라 '
                    '적고 비워 둡니다 — 한 사람의 아이 이야기가 학원의 성격으로 읽히면 안 되기 '
                    '때문입니다.\n\n'
                    '이 문장들은 점수·순위·정렬 어디에도 쓰지 않습니다. 산식은 이 칸을 보지 않고, '
                    '카드는 우리 아이에게 맞는 곳인지 가늠하는 데만 씁니다. 어느 모델이 언제 썼는지를 '
                    '칸 아래에 밝힙니다.',
                  ),
