...
- '초등' 처럼 구간을 못 정하는 말은 어느 쪽으로도 넘기지 않는다(저·고학년은
  다른 시장).
- **학년은 근거 등급으로 정하고 등급을 학년마다 적는다**(`grade_targets.py`,
  `gradeTarget.bandBasis`): `official` > `directory` > `registered`(공시 명시) >
  `curated`(시드 단계) > `reviews`(후기 반복 언급: 작성자 2명 · 3건 · 그 과목
  학년 언급의 25%). 9/19 에 공시만 인정했더니 채점 581쌍 중 457쌍이 어느
  학년 랭킹에도 없었다(강대·트윈클·렉스김·소마). 넓히되 등급을 적는다.
- **후기 구간은 아는 구간과 이어질 때만**(`_contiguous`). 초등 학원 후기의
  '의대'·'수능까지' 가 고등 25% 를 만든다(소마 초저 54 · 고 24). 떨어진
  구간은 바람이지 반이 아니다.
- 신고가 '학급 매칭이 안 돼 랭킹에 없다' 면 `--from-cache` 로그의
  **`학년 근거 감사`** 줄(학년 없음 N쌍 · 순위권 N)을 본다. 반대 신고('없는
  학급에 있다') 면 그 학원의 `bandBasis` 가 `reviews` 인지 — 후기 노드
  판정·위키가 아니라 문턱·이음 규칙의 문제일 수 있다.
